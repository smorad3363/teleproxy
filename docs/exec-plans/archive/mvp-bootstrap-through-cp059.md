...
Latest verified checkpoint: `c702bec933eed9a86676a36149a246526ffd1753`
...
- CP-059 Web Panel authoritative User exact filters: `c702bec933eed9a86676a36149a246526ffd1753`, CI `34607269536` PASS.
...
- Stage 11R scope docs `deb5bda3...` CI `34606209209` PASS across Format, Vet, full Go tests, installer syntax/unit tests, Docker prerequisites and Telemt E2E/rerun.
- CP-059 candidate `c702bec9...` CI `34607269536` PASS across Format, Vet, full Go tests, installer syntax/unit tests, Docker prerequisites and Telemt E2E/rerun. The candidate changes exactly three production files and adds two focused regression-test files; no migration or unrelated file changed.
...
### Stage 11R — Web Panel authoritative User exact filters — COMPLETED AT CP-059

CP-059 extends only the existing authenticated `GET /api/users` and `GET /users` read contracts with optional exact `telegram_id` and `proxy_username` filters over authoritative SQLite-backed `telegram_users.telegram_id` and `proxy_users.username`. Both filters compose with logical AND; malformed, duplicate, non-positive/non-integer Telegram IDs, invalid proxy usernames and unknown query keys use the established `USER_INVENTORY_INVALID` Problem contract. The `/users` page adds only those two GET filter inputs, preserves lifecycle/reconcile actions and session-derived CSRF behavior, and retains active filters plus explicit limit in Older users pagination. Matching remains exact and case-sensitive with no trimming, fuzzy/substring search, case folding or alternate identity resolution. Empty filtered results are normal read-only results. Candidate `c702bec9...`, CI `34607269536` PASS across Format, Vet, full Go tests, installer syntax/unit tests, Docker prerequisites and Telemt E2E/rerun. No migration, endpoint, Telegram username inference, banned/active policy, Node/Sponsor assignment, traffic/time/credit mutation, referral-tree semantics, audit filtering/writes, lifecycle change, Bot/Telegram behavior, runtime action, per-Node credential/health behavior, reward semantics or Telemt topology change was introduced.

## Current next action

Promote CP-059 documentation only after its candidate gate is recorded, then inspect the roadmap and current repository contracts for the next semantics-established bounded milestone. Preserve Credit Buckets as source of truth and every listed blocker; do not invent fuzzy search, absent user fields, synchronous convergence, polling/retry, proxy-account creation, traffic/time/credit or ban semantics, Node/Sponsor assignment, audit mutation/filter semantics, Dashboard metrics, Administrator RBAC enforcement, Bot runtime composition, per-Node credential/health semantics, referral reward issuance/tree semantics, anti-abuse policy, backup/restore, update/restart, logs, version-source semantics, new Telemt topology or secret persistence.
