...
Latest verified checkpoint: `cbb4ac3a22a18332a74972270ae46a74c381f80d`
...
- CP-058 Web Panel established manual quota reconciliation action: `cbb4ac3a22a18332a74972270ae46a74c381f80d`, CI `34601789910` PASS.
...
- CP-057 added authenticated `/users` browser controls for the already-established CP-009 proxy lifecycle enable/disable and rotate-secret operations. Controls target only the authoritative existing proxy username, send the existing session-derived CSRF header to same-origin lifecycle endpoints, reload authoritative inventory after successful desired-state changes, and reveal a rotated plaintext secret only from the successful response in the current page with an explicit one-time warning. Existing typed failures are bounded for display. No secret is pre-rendered, persisted, logged, audited or auto-copied; no endpoint, migration, proxy-account creation, credit/traffic/time/ban mutation, Node/Sponsor assignment, Telegram/Bot behavior, routing/reward semantics or Telemt topology was added. Candidate `c8ba6e27...`, CI `34596564418` PASS.
- CP-058 added an authenticated `/users` `Reconcile quota` control over the already-established CSRF-protected `POST /api/proxy/users/{username}/reconcile` route. It renders only for rows with a non-empty authoritative proxy username, sends the existing session-derived CSRF header, displays only a bounded `Quota reconciliation queued.` status after `202 Accepted`, and does not poll, reload, retry or claim synchronous convergence. Rows without proxy usernames remain unavailable; existing typed failure handling stays bounded; no secrets or raw upstream errors are exposed. No endpoint, migration, Credit Bucket mutation, proxy-account creation, desired-enabled mutation, traffic/time/ban behavior, assignment/routing/reward semantic, Bot/Telegram behavior, Docker/runtime action or Telemt topology was added. Candidate `cbb4ac3a...`, CI `34601789910` PASS.
...
### Stage 11N — Web Panel read-only Administrator inventory surface — COMPLETED AT CP-055
...
- CP-057 promotion docs `10373fce...` CI `34597108362` PASS across Format, Vet, full Go tests, installer syntax/unit tests, Docker prerequisites and Telemt E2E/rerun.
- CP-058 candidate `cbb4ac3a...` CI `34601789910` PASS across Format, Vet, full Go tests, installer syntax/unit tests, Docker prerequisites and Telemt E2E/rerun. The change is limited to the `/users` page and its regression test; no API/domain/runtime contract changed.
- CP-058 promotion docs `40ce8e43...` CI `34602626633` PASS across Format, Vet, full Go tests, installer syntax/unit tests, Docker prerequisites and Telemt E2E/rerun.
...
### Stage 11Q — Web Panel established manual quota reconciliation action — COMPLETED AT CP-058

CP-058 exposes only the already-established authenticated manual quota reconciliation route on `/users` as an explicit `Reconcile quota` action for rows with a non-empty authoritative proxy username. The browser sends the existing session-derived `X-CSRF-Token` to the exact same-origin `POST /api/proxy/users/{username}/reconcile` endpoint and, on `202 Accepted`, shows only `Quota reconciliation queued.` in the current row. It does not poll, refresh, retry or imply synchronous completion; existing Enable/Disable and Rotate secret behavior remains unchanged, rows without proxy usernames remain safely unavailable, and typed API failures remain bounded. Candidate CI `34601789910` is fully PASS across format/vet/full Go plus installer/Docker prerequisites and Telemt E2E/rerun. No new endpoint, migration, Credit Bucket mutation, traffic/time/ban behavior, desired-enabled mutation, proxy-account creation, Node/Sponsor assignment, audit write, Bot/Telegram behavior, per-Node credentials, routing/reward semantics, Docker/runtime action or Telemt topology was introduced.

### Stage 11R — Web Panel authoritative User exact filters — ACTIVE

The roadmap Users surface explicitly includes search fields for Telegram ID and Proxy Username. Both values already exist in the CP-043 authoritative SQLite-backed user inventory read model, while Telegram username, banned state, assigned Node/Sponsor, general last activity and other roadmap search fields are not established in that model. This milestone adds only exact-match read-only filtering for the two established identifiers.

Scope only:
- extend the existing authenticated `GET /api/users` and `GET /users` query contract with optional single-value `telegram_id` and `proxy_username` parameters in addition to existing `before_id` and `limit`; reject duplicates, non-positive/non-integer Telegram IDs and invalid proxy usernames with the existing `USER_INVENTORY_INVALID` Problem contract;
- interpret both filters as exact matches against authoritative `telegram_users.telegram_id` and `proxy_users.username`; when both are present they compose with logical AND; do not add fuzzy/substring search, case folding, alternate identity resolution or guessed fields;
- preserve newest-first Telegram-user-ID pagination and make `next_before_id` / the Web Panel Older users link retain the active exact filters and explicit limit;
- add a minimal GET filter form to authenticated `/users` for Telegram ID and Proxy Username, preserving current inventory columns, lifecycle/reconcile actions, `Cache-Control: no-store`, session-derived CSRF behavior and safe HTML escaping;
- an empty filter result is a normal empty inventory response/page and must not mutate authoritative state;
- no migration, new endpoint, Telegram username inference, banned/active policy, Node/Sponsor assignment, traffic/time/credit mutation, referral-tree semantics, audit filtering/writes, lifecycle change, Bot/Telegram behavior, runtime action, per-Node credential/health behavior, reward semantics or Telemt topology change.

Acceptance:
- exact Telegram ID and exact Proxy Username each select only matching authoritative rows; combining both requires the same row to match both;
- malformed/duplicate/unknown query keys remain rejected and valid filters remain preserved across pagination;
- `/users` exposes only the two established filter inputs and does not imply support for schema-absent roadmap search fields;
- existing `/api/users` and `/users` behavior without filters remains unchanged;
- targeted `internal/useradmin` and `internal/httpapi` tests plus format/vet/full Go, installer/Docker prerequisites and Telemt E2E/rerun remain green.

## Current next action

Implement Stage 11R exactly as scoped, self-review the complete diff, validate exact filter/pagination semantics, and promote only after the complete CI gate passes. Preserve Credit Buckets as source of truth and every listed blocker; do not invent fuzzy search, absent user fields, synchronous convergence, polling/retry, proxy-account creation, traffic/time/credit or ban semantics, Node/Sponsor assignment, audit mutation/filter semantics, Dashboard metrics, Administrator RBAC enforcement, Bot runtime composition, per-Node credential/health semantics, referral reward issuance/tree semantics, anti-abuse policy, backup/restore, update/restart, logs, version-source semantics, new Telemt topology or secret persistence.
