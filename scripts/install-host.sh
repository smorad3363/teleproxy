...
for cmd in docker curl awk od tar cp mv mkdir chmod chown flock hostname install seq grep tr rm sleep; do
...
mkdir -p "$STATE_DIR" "$DATA_DIR" "$SECRETS_DIR"
chmod 0755 "$STATE_DIR"
exec 9>"$LOCK_FILE"
flock -n 9 || { echo "another Teleproxy installation is already running" >&2; exit 1; }

# Preserve immutable choices from a previous installation.
...
# UID/GID 10001 matches the non-root user baked into the control image.
...
existing_id=$(compose ps -q control 2>/dev/null || true)
if [[ -n "$persisted_port" && -z "$existing_id" ]] && port_in_use "$PANEL_PORT"; then
...
printf 'Proxy Plane:      not configured yet\n'
printf 'Source Ref:       %s\n' "$SOURCE_REF"
printf 'Install Path:     %s\n' "$INSTALL_DIR"
printf 'Data Path:        %s\n' "$DATA_DIR"
printf 'Management:       tproxy status | tproxy logs | tproxy restart | tproxy panel\n'
printf '%s\n' '----------------------------------------'
printf '%s\n' 'Security note: the Stage 5 panel is HTTP-only; TLS is added in a later hardening stage.'

if [[ "$credential_printed" == "0" ]]; then
  credential_printed=1
fi
write_install_state "$STATE_FILE" installed "$PANEL_PORT" "$PANEL_BIND" "$ADMIN_USER" "$SOURCE_REF" "$credential_printed" "$DATA_DIR" "$SECRETS_DIR" "$CONTROL_IMAGE"
rm -f "$password_file"
