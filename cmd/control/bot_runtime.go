...
type telegramBotRuntime struct {
	webhook http.Handler
	poller  *telegrambot.Poller
}

func configuredTelegramRuntime(ctx context.Context, db *sql.DB, legacyUsername string, proxyClient *telemt.Client, quotaRunner *quotareconcile.Runner) (telegramBotRuntime, bool, error) {
	value, configured, err := settings.BotRuntime(ctx, db)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("load Telegram Bot runtime settings: %w", err)
	}
	username := ""
	if configured {
		if !value.Enabled {
			return telegramBotRuntime{}, false, nil
		}
	...
	if proxyClient == nil || quotaRunner == nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram Bot proxy provisioning: Telemt quota reconciliation is required")
	}

	tokenFile, secretFile := runtimeBotSecretPaths()
	botClient, err := telegrambot.NewFromTokenFile(tokenFile, 3*time.Second)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram Bot client: %w", err)
	}
	webhookSecret, err := telegrambot.LoadWebhookSecretFile(secretFile)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram webhook authentication: %w", err)
	}
	provisioner, err := proxyprovision.NewService(db, proxyClient, quotaRunner, nil)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram proxy provisioning: %w", err)
	}
	startApplication, err := telegrambot.NewStartApplicationWithForcedJoin(db, username, nil, provisioner, botClient)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram start application: %w", err)
	}
	webhook, err := telegrambot.NewWebhookHandler(webhookSecret, startApplication, botClient)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram webhook: %w", err)
	}
	poller, err := telegrambot.NewPoller(botClient, startApplication, botClient)
	if err != nil {
		return telegramBotRuntime{}, false, fmt.Errorf("configure Telegram polling: %w", err)
	}
	return telegramBotRuntime{webhook: webhook, poller: poller}, true, nil
}
