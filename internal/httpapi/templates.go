...
<body><header><strong>Teleproxy</strong><form method="post" action="/logout"><input type="hidden" name="csrf" value="{{.CSRF}}"><button type="submit">Sign out</button></form></header><main><section class="card"><h1>Control Plane</h1><p>Signed in as <strong>{{.Username}}</strong>.</p><p><a href="/sponsors">Manage Sponsor Profiles</a></p><p><a href="/nodes">Manage Proxy Nodes</a></p><p><a href="/referrals">Manage Referrals</a></p><p><a href="/forced-join">Manage Forced Join</a></p><p class="muted">Core login is active. Additional Proxy, Bot and plan management will be added in later stages.</p></section></main></body></html>`))
